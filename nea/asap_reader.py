...
def read_dataset(file_path, prompt_id, maxlen, vocab, tokenize_text, to_lower, score_index=6, char_level=False):
    ...
    return data_x, data_y, prompt_ids, maxlen_x, essay_ids
...
def get_data(paths, prompt_id, vocab_size, maxlen, tokenize_text=True, to_lower=True, sort_by_len=False,
    ...
    train_x, train_y, train_prompts, train_maxlen, train_essay_ids = read_dataset(train_path, prompt_id, maxlen, vocab,
                                                                                  tokenize_text,
                                                                                  to_lower)
    dev_x, dev_y, dev_prompts, dev_maxlen, dev_essay_ids = read_dataset(dev_path, prompt_id, 0, vocab, tokenize_text,
                                                                        to_lower)
    test_x, test_y, test_prompts, test_maxlen, test_essay_ids = read_dataset(test_path, prompt_id, 0, vocab,
                                                                             tokenize_text, to_lower)

    overal_maxlen = max(train_maxlen, dev_maxlen, test_maxlen)

    return (
        (train_x, train_y, train_prompts, train_essay_ids), (dev_x, dev_y, dev_prompts, dev_essay_ids),
        (test_x, test_y, test_prompts, test_essay_ids), vocab,
        len(vocab),
        overal_maxlen, 1)
